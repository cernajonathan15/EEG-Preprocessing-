...
# Disparity filter + determining the optimal alpha
...
#windowed_cross_correlation_matrices = [] --> WILL CHANGE THIS FOR JUST A PLI
...
num_of_windows_dpli = len(windowed_dpli_matrices)
num_of_windows_wpli = len(windowed_wpli_matrices)

print(f"Total number of windows for dPLI: {num_of_windows_dpli}")
print(f"Total number of windows for wPLI: {num_of_windows_wpli}")

# Calculating the functional (cross-correlation) and directional (dPLI) connectivity across all windows (or choose a specific window)
#chosen_window = 0 # Choose a window
wPLI_matrix = np.mean(windowed_wpli_matrices, axis=0)
dPLI_matrix = np.mean(windowed_dpli_matrices, axis=0) # can do windowed_dpli_matrices[chosen_window] to only choose one window
G_dPLI_matrix_list = [nx.convert_matrix.from_numpy_array(matrix) for matrix in windowed_dpli_matrices] # can do windowed_dpli_matrices[chosen_window] to only choose one window
G_wPLI_matrix_list = [nx.convert_matrix.from_numpy_array(matrix, create_using=nx.Graph) for matrix in windowed_wpli_matrices] # can do windowed_wpli_matrices[chosen_window] to only choose one window

# Thresholding the connectivity matrix, fc and directional
#avg_connectivity_matrix_thresholded = disparity_filter(avg_connectivity_matrix, alpha=optimal_alpha)  # Use the optimal alpha value here --> WILL CHANGE THIS FOR JUST A PLI
G_dPLI_thresholded_list = [disparity_filter(G, alpha=optimal_alpha) for G in G_dPLI_matrix_list]  # Use the optimal alpha value here
G_wPLI_thresholded_list = [disparity_filter(G, num_iterations=1000, percentile=95) for G in G_wPLI_matrix_list]

# If you need to convert the thresholded graphs back to matrices for each window
dPLI_matrix_thresholded_list = [nx.convert_matrix.to_numpy_array(G) for G in G_dPLI_thresholded_list]
wPLI_matrix_thresholded_list = [nx.convert_matrix.to_numpy_array(G) for G in G_wPLI_thresholded_list]
