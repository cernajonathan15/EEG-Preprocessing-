...
# Boostrapped Disparity Filter use for data-driven thresholding of the graph
def disparity_filter(G, alpha=0.05, num_iterations=1000):
    # Get edge weights from the graph
    edge_weights = [data['weight'] for _, _, data in G.edges(data=True)]
    max_weight = max(edge_weights)

    # Compute the disparity measure (Y)
    Y = [(w / max_weight) for w in edge_weights]

    # Perform bootstrapping
    num_edges = len(edge_weights)
    bootstrap_Y = []
    for _ in range(num_iterations):
        random_weights = np.random.choice(edge_weights, size=num_edges, replace=True)
        max_random_weight = max(random_weights)
        bootstrap_Y.extend([(w / max_random_weight) for w in random_weights])

    # Compute threshold for each edge based on the disparity measure
    edge_thresholds = {}
    for idx, (_, _, data) in enumerate(G.edges(data=True)):
        edge_Y = Y[idx]
        p_value = sum(1 for y in bootstrap_Y if y >= edge_Y) / num_iterations
        if p_value < alpha:
            edge_thresholds[(u, v)] = data['weight']

    # Create a thresholded graph
    if isinstance(G, nx.DiGraph):
        G_thresholded = nx.DiGraph()
    else:
        G_thresholded = nx.Graph()

    for (u, v), weight in edge_thresholds.items():
        G_thresholded.add_edge(u, v, weight=weight)

    return G_thresholded

G_dPLI_thresholded = disparity_filter(G_dPLI, directed=True)
G_PLI_thresholded = disparity_filter(G_PLI, directed=False)
