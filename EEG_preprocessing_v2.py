...
# Update the location of FPz (GND) in the original_EEG object
...
# Plot the explained variance ratio (if needed)
...
n_components = 0.99  # Initially, set to 0.99 to let ICA determine components based on explained variance

# Create an ICA instance with the initial n_components
ica = ICA(
...
# Check the actual number of components selected
n_components_actual = ica.n_components_

# If fewer than 15 components were selected, re-run ICA with 15 components
if n_components_actual < 15:
    ica = ICA(
        n_components=15,  # Set to exactly 15 components
        max_iter="auto",
        method="infomax",
        random_state=97,
        fit_params=dict(extended=True),
    ...
    print(f"Selected {n_components_actual} components")
