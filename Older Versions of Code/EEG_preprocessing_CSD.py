...
original_EEG = EEG_csd

# Crop a copy of the data to three seconds for easier plotting
cropped_EEG = EEG_csd.copy().crop(tmin=0, tmax=3).load_data()
...
pca = PCA(n_components=0.90)  # Change to 0.99 or 0.90 based on your preference
pca.fit(data)
n_pca = pca.n_components_

# Define ICA parameters
n_components = n_pca  # Use the number of PCA components for ICA
ica = ICA(
...
for i in range(0, n_components_actual, 62):
    ica.plot_components(picks=range(i, min(i + 62, n_components_actual)), ch_type='eeg', inst=original_EEG)
    # blinks
