...
# Save the computed inverse operator to a .fif file
inverse_operator_file = os.path.join(output_dir, subj + '_inverse_operator.fif')
mne.minimum_norm.write_inverse_operator(inverse_operator_file, inv)

# Compute eLORETA solution for each epoch
snr = 3.0
lambda2 = 1.0 / snr**2
stcs = apply_inverse_epochs(epochs, inv, lambda2, "eLORETA", verbose=True, pick_ori="vector") # pick_ori="vector" --> loose dipole orientations
...
# Visualizing the averaged source estimate with loose dipole orientations
brain_loose = stc_avg.plot(**kwargs)
mne.viz.set_3d_view(figure=brain_loose, focalpoint=(0.0, 0.0, 50))

# Average the data across all source space points and orientations
